...
function restart() {
    ...
    round = 0;
    player = 0;
    for (const key1 of plane){
        const box = document.querySelector(`.box_${key1}`);
...
const hidden = (player1, draw = false)=>{
    page.classList.remove("hidden");
    overlay.classList.remove("hidden");
    if (!draw) p.textContent = !player1 ? "Congratulations! Player O Win!" : "Congratulations! Player X Win!";
...
for (const key of plane){
    ...
    box.addEventListener("click", ()=>{
        if (!(`${key}` in flat)) {
            ...
            changePlayer();
        } else checkWin(player, flat, round);
    });
}
function checkWin(player2, f, r) {
    ...
    if (round === 9 && !win) hidden(player2, true);
}
